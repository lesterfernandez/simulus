...
class _Event(Trappable):
    ...
    def __str__(self):
        return "%g: evt=%s" % (self.time, self.name if self.name else id(self))
...
class _DirectEvent(_Event):
    ...
    def __str__(self):
        return "%g: dir_evt=%s %s" % (
            self.time,
            self.name if self.name else self.func.__name__ + "()",
...
class _ProcessEvent(_Event):
    ...
    def __str__(self):
        return "%g: prc_evt=%s" % (
            self.time,
            self.name if self.name else self.proc.func.__name__ + "()",
        )
...
class _EventList_(object):
    ...
    def insert(self, evt):
        if self.last <= evt.time:
            ...
            raise ValueError(
                "EventList.insert(%s): past event (last=%g)" % (evt, self.last)
            )
    ...
    def update(self, evt):
        ...
        if self.last <= evt.time:
            ...
            raise ValueError(
                "EventList.update(%s): past event (last=%g)" % (evt, self.last)
            )
